...
import { getEndpoint, getToken } from 'zero-element-boot/lib/components/config/common'; // 引入自己修改的样式
...
export default function index(props) {
  ...
  const change = (files, type, index) => {
    ...
    uploadImage('/api/u/fs/uploadfile', files[0].file).then(resp => {
      if (resp && resp.code === 200) {
        callBack(resp.data.url); // console.log('url',resp.data.url)
      }
    });
    setAvator(files);
  };
  ...
  */
  ...
  function uploadImage(url, file) {
    ...
    payload.append('file', file); // console.log('payload=', payload);
